...
function buildColumn(container, items, selectedValue) {
    ...
    items.forEach((item) => {
        ...
        btn.type = 'button';
        btn.className = 'scroll-picker-item w-full shrink-0 snap-center tabular-nums';
...
function createScrollPicker(root) {
    ...
    function renderColumns() {
        ...
        if (type === 'date') {
            ...
            specs.forEach((spec) => {
                ...
                col.className = 'scroll-picker-column flex-1';
    ...
    function normalizeDraft() {
        if (type === 'date') {
            const min = minDateObj();
            let date = new Date(Number(draft.year), Number(draft.month) - 1, Number(draft.day));
    ...
    function commitValue(value) {
        hidden.value = value;
        updateDisplay(value);
        root.dispatchEvent(new CustomEvent('scroll-picker:change', { detail: { value } }));
    }
...
export { initScrollPickers, roundTimeUpToFiveMinutes, formatDMY, formatISO, parseDMY };
